...
class MulLayer(nn.Module):
    ...
    def forward(self,cF,sF,alpha,trans=True):
        ...
        if(trans):
            ...
            out = out + sMeanC
            #mustafa testing
            # alpha = 0.5  # 0 = only content, 1 = full style

            styled = out
            origin = cF  # original content feature (before mean removal)

            out2 = alpha * styled + (1 - alpha) * origin

            return out, transmatrix, KL,out2
        else:
