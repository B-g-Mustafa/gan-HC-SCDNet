...
def upload_checkpoints(local_folder, drive_folder_id):
    ...
    try:
        ...
        # 2. Helper: create or get a folder in Drive
        def get_or_create_folder(parent_id, folder_name):
            """Get existing folder or create a new one under parent."""
            try:
                ...
                q = (f"name = '{folder_name}' and "
                     ...
                     f"trashed = false")
                response = service.files().list(q=q, spaces='drive', fields='files(id, name)', pageSize=10).execute()
                ...
                print(f"  └─ Created Drive folder: '{folder_name}'", flush=True)
                return created.get('id')
            except Exception as e:
                print(f"Warning: Could not create/get folder '{folder_name}': {e}", flush=True)
                return None

        # 3. Get a map of all existing files in the Drive folder (with their paths)
        def get_all_files_in_folder(folder_id, path=""):
            """Recursively get all files in a Drive folder and its subfolders, maintaining path."""
            all_files = {}
            try:
                ...
                for item in items:
                    ...
                    if item['mimeType'] == 'application/vnd.google-apps.folder':
                        ...
                        # It's a file, add to dict with full path as key
        ...
        # 4. Recursively collect all local files from source and subfolders, preserving relative paths
        local_files_to_upload = []
        local_root = os.path.abspath(local_folder)
        for root, dirs, files in os.walk(local_folder):
            for filename in files:
                local_filepath = os.path.join(root, filename)
                # Calculate relative path from the root local folder
                rel_path = os.path.relpath(local_filepath, local_folder)
                local_files_to_upload.append((filename, local_filepath, rel_path))

        print(f"Found {len(local_files_to_upload)} files to process in local folder (including subfolders).\n")

        # 5. Upload files, creating folder structure and skipping duplicates
        uploaded_count = 0
        skipped_count = 0
        failed_count = 0
        folder_cache = {}  # Cache Drive folder IDs by their path

        for filename, local_filepath, rel_path in local_files_to_upload:
            try:
                # Determine target parent folder in Drive
                parent_id = drive_folder_id
                rel_dir = os.path.dirname(rel_path)  # Get folder path without filename
                
                # Create folder hierarchy if needed
                if rel_dir and rel_dir != '.':
                    folder_path_parts = rel_dir.split(os.sep)
                    for folder_name in folder_path_parts:
                        cache_key = os.path.join(str(parent_id), folder_name)
                        if cache_key not in folder_cache:
                            new_folder_id = get_or_create_folder(parent_id, folder_name)
                            if new_folder_id:
                                folder_cache[cache_key] = new_folder_id
                            else:
                                print(f"  ✗ Failed to create folder: {folder_name}", flush=True)
                                failed_count += 1
                                continue
                ...
                if rel_path in existing_drive_files:
                    print(f"⊘ Skipping '{rel_path}' -> Already exists in Google Drive")
                    skipped_count += 1
                    continue
                ...
                print(f"  ✓ Successfully uploaded '{rel_path}' (ID: {response.get('id')})")
                uploaded_count += 1
            except Exception as e:
