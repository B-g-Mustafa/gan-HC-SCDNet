...
#SBATCH --output=/home/msai/birul001/gan-project/gan-HC-SCDNet/ccds-files/logs/train/output_%x_%j.out   # Std output log
#SBATCH --error=/home/msai/birul001/gan-project/gan-HC-SCDNet/ccds-files/logs/train/error_%x_%j.err     # Std error log
...
# -----------------------------
PYTHON_SCRIPT="/home/msai/birul001/gan-project/gan-HC-SCDNet/b-vae/train/train_beta_vae.py"  # <-- your Python script path
...
OUTPUT_TXT="${CCDS_DIR}/train_output/output_${RUN_TS}_${JOB_ID}.txt"
{
...
if [ -f "$PYTHON_SCRIPT" ]; then
    echo "Executing: python $PYTHON_SCRIPT --content_dir $CONTENT_PATH --style_dir $STYLE_PATH" >> "$OUTPUT_TXT"
    python "$PYTHON_SCRIPT" --content_dir "$CONTENT_PATH" --style_dir "$STYLE_PATH" --val_style_dir "$STYLE_PATH_val" --val_content_dir "$CONTENT_PATH_val" --save_dir "$CHECKPOINT_PATH" >> "$OUTPUT_TXT" 2>&1
else
